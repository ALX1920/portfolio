...
Efecto: La pestaña nueva podía acceder a window.opener de la pestaña original (vulnerabilidad conocida como tabnabbing), y en navegadores antiguos también afectaba el rendimiento.
...
components/buttons.css .btn, .social-link, .chip, .badge, enlaces (cv**link, project-card**link, contact\_\_link, etc.) Todo lo interactivo/clicable en un solo lugar
...
.section/.project-card/.blog-card/.skills-block con opacity: 0 + transform: translateY(16px) como estado inicial, y .visible que los regresa a su estado normal con una transición suave. 3. script.js
...
3.6 Selector fragil del botón "Ver más proyectos"
...
<!DOCTYPE html> <html lang="es"> <head> <meta charset="UTF-8"> <!-- Hace que la web sea responsive en móvil --> <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>Portafolio | Alejandro Martínez</title> <!-- Enlace al archivo de estilos principal -->

PASTED

/_ ========================================== RESET BÁSICO Y TIPOGRAFÍA GLOBAL ========================================== _/ \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; backgrou

PASTED

/_ RESET BÁSICO Y TIPOGRAFÍA GLOBAL _/ \* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; background-color: var(--bg-main); color: var(--text-main); line-height: 1.6; } ul {

PASTED

/_ CV _/ .cv { padding: 18px 16px; border-radius: var(--radius-card); border: 1px solid var(--border-soft); background: rgba(15,23,42,0.8); } .cv\_\_hint { margin-top: 6px; font-size: 0.8rem; color: var(--text-muted); } /_ PROYECTOS _/ .projects-grid { display: grid;

PASTED

/\* ============================================================ ARCHIVO: script.js AUTOR: Alejandro Martínez DESCRIPCIÓN: Funciones ligeras para mejorar la experiencia de usuario (UX) sin saturar la página ni agregar complejidad innecesaria. =======================================
